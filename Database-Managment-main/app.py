...
def init_db():
    ...
    cursor.execute('''
                DROP TABLE IF EXISTS emp_review
            ''')
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.execute('''
            ...
            review_score FLOAT(1),
            review_comment VARCHAR(1000),
            PRIMARY KEY (cust_id, emp_id)
            FOREIGN KEY (cust_id) REFERENCES cust_info
            FOREIGN KEY (emp_id) REFERENCES emp_info
            )
        ''')

    # Check if any reviews exist for employees, and insert sample data if none exists
    cursor.execute('SELECT COUNT(*) FROM emp_review')
    if cursor.fetchone()[0] == 0:
        cursor.executemany('''
            INSERT INTO emp_review (cust_id, emp_id, review_score, review_comment)
            VALUES (?, ?, ?, ?)
        ''', [
            ('CUST366448', 'emp1', 4.5,
             "John is extremely knowledgeable and helped me find the perfect car within my budget.")
        ])
...
def employees():
    if current_user_id:
        current_user = get_username(current_user_id)
        return render_template('employee.html').replace('var currentUser = null;', f'var currentUser = "{current_user}";')
...
def employee_detail(emp_id):
    if current_user_id:
        current_user = get_username(current_user_id)
        return render_template('employee_detail.html').replace('var currentUser = null;', f'var currentUser = "{current_user}";')
...
@app.route('/api/employee/<emp_id>/reviews', methods=['GET'])
def get_employee_reviews(emp_id):
    conn = get_db_connection()
    cursor = conn.cursor()

    # Fetch reviews for the given employee ID
    cursor.execute('SELECT cust_id, review_score, review_comment FROM emp_review WHERE emp_id = ?', (emp_id,))
    reviews = cursor.fetchall()

    # Convert reviews to a list of dictionaries
    review_list = [
        {
            'cust_id': review['cust_id'],
...
# API to handle review submission and update
@app.route('/api/review/<emp_id>', methods=['POST'])
def submit_review(emp_id):
    data = request.json
    cust_id = data.get('cust_id')
    review_score = data.get('review_score')
    review_comment = data.get('review_comment')

    if not cust_id or not review_score or not review_comment:
        return jsonify({'error': 'All fields are required'}), 400

    conn = get_db_connection()
    cursor = conn.cursor()

    # Check if the customer already reviewed this employee
    cursor.execute('SELECT * FROM emp_review WHERE cust_id = ? AND emp_id = ?', (cust_id, emp_id))
    ...
    if existing_review:
        ...
        cursor.execute('''
            UPDATE emp_review
            SET review_score = ?, review_comment = ?
            WHERE cust_id = ? AND emp_id = ?
        ''', (review_score, review_comment, cust_id, emp_id))
    else:
        # Insert a new review
        cursor.execute('''
            INSERT INTO emp_review (cust_id, emp_id, review_score, review_comment, review_date)
            VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
        ''', (cust_id, emp_id, review_score, review_comment))

...
async function loadEmployeeReviews() {
    try {
        ...
        const response = await fetch(`/api/employee/${employeeId}/reviews`);
...
window.addEventListener('load', loadEmployeeReviews);
...
async function saveReview() {
    ...
    const response = await fetch(`/api/employee/${employeeId}/review`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ review_score: reviewScore, review_comment: reviewComment })
    });
